...
Например, если пользователь загрузит для обработки координаты, указывающие в 500 метрах от г. Выкса на лес, то маркер на карте OSM встанет точно в лесу, а подписан он будет примерно так: 'Координаты: 55.3301 -42.2604::Страна:RU::Городской_округ_1:Nizjnij Novgorod::Городской_округ_2:Vyksa'. То есть метод работает на основе — 'Евклидово дерево'.

\033[32;1mПлагин Reverse Vgeocoder\033[0m \033[32m- работает в оффлайн режиме и укомплектован гео-БД (БД предоставляются под свободной лицензией от download.geonames.org/export/dump/).

Для визуализации данных на карте OSM укажите (при запросе) текстовый файл с координатами в кодировке utf-8 (с расширением .txt или без расширения). Каждая строчка с геокоординатами (широта, долгота) должна быть записана в файле с новой строки (желательно).
...
Пример файла с координатами (как может выглядеть файл с координатами, который необходимо указывать):
\033[36m
51.352, 108.625
55.466,64.776
52.40662,66.77631
...
статистические отчеты в [txt/csv/html и визуализированные данные на карте OSM].
