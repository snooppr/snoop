...
логины пользователей; их имена; e-mail's и их персональные ссылки на сервисы Яндекса. А так же доступен txt-отчет с расширенными, структурированными данными о пользователях.
...
По окончанию успешного поиска выводится отчёт в CLI, сохраняется расширенный txt-отчёт о Яндекс-пользователях и открывается браузер с мини-отчётом.
...
\033[32mРежим '\033[32;1mOffline поиск\033[0m\033[32m'. Модуль GEO_IP/domain от Snoop Project использует специальные базы данных и создает статистическую и визуализированную информацию только по ip цели (массиву данных).
    (Базы данных доступны свободно для скачивания от компании Maxmind после регистрации аккаунта, ограничения: скачанные базы можно использовать для личных целей или внутри организации. По лицензии Maxmind БД нельзя распространять разработчикам, поэтому БД не поставляются со Snoop Project).
Скачать бесплатно базы\033[0m \033[32;1mhttps://dev.maxmind.com/geoip/geoip2/geolite2/\033[0m
\033[32mДля использования поиска необходимо скачать две бесплатные базы: ~40Мб '\033[0m\033[32;1mGeoLite2-City.mmdb\033[0m\033[32m' и '\033[0m\033[32;1mGeoLite2-ASN.mmdb\033[0m\033[32m'.
...
Режим '\033[32;1mOffline_тихий поиск\033[0m\033[32m':: Тот же режим, что и режим 'Offline', но не выводит на печать промежуточные таблицы с данными. Режим даёт прирост производительности в несколько раз.
Пример файла с данными (список.txt):
